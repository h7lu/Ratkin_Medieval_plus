...
{
    /// <summary>
    /// Job driver for eating from the Big Pot (Building_FoodGetter).
    /// Dispenses food into hand, then finds a place to sit and eat.
    /// </summary>
    public class JobDriver_EatFromBigPot : JobDriver
    {
        private Building_FoodGetter BigPot => job.GetTarget(TargetIndex.A).Thing as Building_FoodGetter;
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            gotoPot.FailOn(() => BigPot != null && !BigPot.CanDispenseNow_New);
            ...
            dispense.initAction = () =>
            {
                Building_FoodGetter pot = BigPot;
                if (pot == null || !pot.CanDispenseNow_New)
                {
